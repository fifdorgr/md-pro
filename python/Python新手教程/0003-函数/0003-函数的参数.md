...
现在，如果我们要计算x^3怎么办？可以再定义一个power3函数，但是如果要计算x4、x5……怎么办？我们不可能定义无限多个函数。
...
这个时候，默认参数就排上用场了。由于我们经常计算x^2，所以，完全可以把第二个参数`n`的默认值设定为`2`：
...
而对`于n > 2`的其他情况，就必须明确地传入`n`，比如`power(5, 3)`。
...
Python函数在定义的时候，默认参数`L`的值就被计算出来了，即[]，因为默认参数L也是一个变量，它指向对象`[]`，每次调用该函数，如果改变了`L`的内容，则下次调用时，默认参数的内容就变了，不再是函数定义时的`[]`了。
